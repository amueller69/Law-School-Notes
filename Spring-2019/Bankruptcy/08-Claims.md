...
* **§506(a) – Scope of a secured claim**
  ...
  * **§506(a)(2)** – Waives Rash in Chapter 7, 13 involving individuals when dealing with personal property (not real estate)
...
* **Rash – look to see what the debtor intends to use the property for**: If debtor intends to keep property, then use the replacement value. If debtor intends to abandon property, then use foreclosure value
  * Facts: Debtor exercises its “cram down” power. §1325(a)(5) - Debtor in a Ch. 13 case is permitted to keep property over objection of creditor. In which case, creditor retains a lien securing the claim
  * Holding: under §506(a), value of property retained because of debtor’s exercising cram down power is cost debtor would incur to obtain comparable collateral (replacement value)
  * Appropriate to use replacement value (higher value) because creditor is exposed to higher risks
...
* Valuation Hypo:
  ...
  * Question: What is the secured creditor’s claim?
    ...
    * What is the pay out for Bank’s 10k unsecured claim?
      * Pro-rata calculation:
        * (10k)/(10+90k) (25k) = 2.5k
        * Amount of your claim/amount of all claims X amount of debtor’s remaining assets
        * Note: this is different than outside of bankruptcy which is a first come/first serve payout
  * Question: If the value of the tractor is unknown. Which is the correct value of the tractor?
    ...
    * Rash approach: Depends on what the debtor is going to do with the property…
      * If in Ch. 11 (debtor is going to keep the property/using the cramdown or stripdown power)…then it will be the replacement value
      * If debtor is abandoning….use the foreclosure value (foreclosure value minus cost)
...
* Dewsnup reading of §506(d):
  * §506(d) does not void a lien in excess of amount of secured claim. Instead, under Dewsnup, a lien passes through bankruptcy. Now, after Caulkett, even a totally under-water claim (like the second lender in Hypothetical below) passes through bankruptcy and can’t be stripped off under Dewsnup  (Skeel: bad court decision)
    ...
    * §363—provision authorizing sale of property inside bankruptcy. Generally extinguishes liens
  ...
  * Note: Dewsnup view only applies in Ch. 7 case (not 11 or 13). Why? It would negatively impact debtor’s ability to successfully reorganize and there are provisions in Ch. 11 and 13 that clearly imply debtor being able to reorganize over-collateralized lenders
...
* Hypo:
  * Setup - Debtor is a small business in chapter 7
    ...
    * Trustee abandons tractor
    * Sales Price: 75k (15k more than court valued property)
  * Question: who gets the extra 15k?
    * Dewsnup: Bank gets the 15k because the court’s valuation of the secured claim is not binding (but would still have a 25K unsecured claim; reduced from 40K)
    * Non-Dewsnup: would give the remaining value to the debtor. Under this approach, once the court values the claim, it is fixed at that amount and we do not go back and change it
  ...
  * What if: property valued at 60k actually sold for 50k?
    ...
    * Non-Dewsnup word…once we void the unsecured claim (under §506(d)), it is permanent (tough luck Bank). No readjustment because the separation of the claim into secured and unsecured portions (bifurcation) is permanent
...
* Over-secured creditors are entitled to interest up until the value of the collateral
  * Interest to be calculated using the contract rate generally
  * Interest is capped at the “equity cushion” (collateral value – claim amount)
    * Ex: If collateral is worth 110k and Bank owed 100k. Bank can only get up to 10k in interest
  * Concerns post-petition claims (502b2 worried about pre-petition)
...
* Ron Pair: (Comma case) an over-secured creditor is entitled to interest regardless of whether the lien is consensual/voluntary
  ...
  * But…only entitled to fees if it says so in the written agreement
* Under-secured creditors do NOT get interest (or anything that might resemble interest)
